...
class AuthenticationTest extends AbstractTestCase
{
    ...
    public function testLogin(): void
    {
        ...
        // Test before login
        $client->request('POST', $this->getApiRoutePrefix() . 'graphql', ['json' => $listQuery]);
        $this->assertJsonContains(['data' => []]);
        $response = $client->request('POST', $this->getApiRoutePrefix() . 'graphql', ['json' => $createQuery]);
        ...
        $client->request('POST', $this->getApiRoutePrefix() . 'graphql', ['json' => $listQuery]);
        $this->assertJsonContains(['data' => []]);
        $client->request('POST', $this->getApiRoutePrefix() . 'graphql', ['auth_bearer' => $token, 'json' => $createQuery]);
        $this->assertEquals('Access Denied.', $response->toArray()['errors'][0]['message']);
