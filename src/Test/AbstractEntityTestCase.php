        ...
        $expectedResponse = new ExpectedResponse(
            ...
            function (ResponseInterface $response) use ($data, $validRegex) {
                ...
                $this->assertJsonContains(
                    array_merge(
                        [
                            '@context' => "{$this->getApiRoutePrefix()}contexts/$shortName",
                            '@type' => $shortName,
                        ],
        ...
        $expectedResponse = new ExpectedResponse(
            ...
            function (ResponseInterface $response) use ($expectedData) {
                ...
                if ($response->getStatusCode() < 400) {
                    ...
                    $this->assertJsonContains(['@context' => $this->getApiRoutePrefix() . "contexts/$shortName", '@type' => $shortName]);
                }
            }
        );
                ...
                if ($response->getStatusCode() < 400) {
                    $this->assertJsonContains(
                        array_merge(
                            [
                                '@context' => "{$this->getApiRoutePrefix()}contexts/$shortName",
                                '@id' => $this->getApiRoutePrefix() . $this->getApiPath(),
                    ...
                    $this->assertJsonContains(['@context' => "{$this->getApiRoutePrefix()}contexts/$shortName", '@type' => 'hydra:Collection']);
                }
            }
        );
